...
test("canonical task ordering is persistent and replaces client sort modes", async () => {
  ...
  const reorderChunkSize = Number(database.match(/TODO_REORDER_ROWS_PER_STATEMENT = (\d+)/)?.[1]);
  assert.ok(Number.isInteger(reorderChunkSize));
  assert.ok(reorderChunkSize * 3 <= 100, "each reorder row binds three values and D1 permits at most 100 per query");
  assert.match(database, /index \+= TODO_REORDER_ROWS_PER_STATEMENT/);
  assert.match(database, /maxBoundParameters: Math\.min\(changed\.length, TODO_REORDER_ROWS_PER_STATEMENT\) \* 3/);
  ...
  assert.match(route, /errorMessage: message/);
