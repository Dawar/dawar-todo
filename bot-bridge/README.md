...
Bots default to `gpt-6-luna`, high reasoning and the catalog's Fast tier (`priority` in Codex 0.156.1). Per-bot model, effort and Fast controls override these defaults. Delegated tasks capture the manager bot's effective settings; explicit `model`, `effort` or `serviceTier` task arguments override them, including when a specialist thread is reused. `serviceTier: "default"` selects standard speed. The global `~/.codex/config.toml` is unchanged.
